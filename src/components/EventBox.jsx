const EventBox = ({ date, title, location }) => {
  return (
    <div className="w-full shadow-shadow3  flex justify-start items-start flex-col p-5 rounded-lg bg-white gap-4">
      <div className="flex justify-start items-center gap-3">
        <p className="w-min text-center rounded-lg min-w-[60px] px-2 aspect-square bg-darkIndigo flex justify-center items-center uppercase text-white text-[15px]">
          {date}
        </p>
        <p className="text-[17px]  text-nightBlue font-medium leading-[1.3]">
          {title}
        </p>
      </div>
      <div className="flex justify-start items-center gap-3">
        <svg
          width="16"
          height="16"
          viewBox="0 0 16 16"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <g clipPath="url(#clip0_108_137)">
            <path
              d="M10.6711 10.5714C12.3737 7.89975 12.1597 8.23306 12.2087 8.16341C12.8286 7.28909 13.1562 6.26006 13.1562 5.1875C13.1562 2.34313 10.8481 0 8 0C5.16119 0 2.84375 2.3385 2.84375 5.1875C2.84375 6.25938 3.17825 7.31534 3.81844 8.20144L5.32881 10.5714C3.71397 10.8196 0.96875 11.5591 0.96875 13.1875C0.96875 13.7811 1.35619 14.627 3.20194 15.2862C4.49075 15.7465 6.19472 16 8 16C11.3758 16 15.0312 15.0478 15.0312 13.1875C15.0312 11.5588 12.2893 10.8201 10.6711 10.5714ZM4.60153 7.68578C4.59637 7.67772 4.591 7.66984 4.58537 7.66209C4.05266 6.92922 3.78125 6.06066 3.78125 5.1875C3.78125 2.84319 5.66894 0.9375 8 0.9375C10.3262 0.9375 12.2188 2.84403 12.2188 5.1875C12.2188 6.06206 11.9525 6.90116 11.4486 7.61472C11.4034 7.67428 11.639 7.30828 8 13.0184L4.60153 7.68578ZM8 15.0625C4.31269 15.0625 1.90625 13.9787 1.90625 13.1875C1.90625 12.6558 3.14275 11.7814 5.88275 11.4406L7.60469 14.1426C7.69075 14.2776 7.83981 14.3594 7.99997 14.3594C8.16012 14.3594 8.30922 14.2776 8.39525 14.1426L10.1172 11.4406C12.8572 11.7814 14.0938 12.6558 14.0938 13.1875C14.0938 13.9719 11.709 15.0625 8 15.0625Z"
              fill="#6A7A99"
            />
            <path
              d="M8 2.84375C6.70766 2.84375 5.65625 3.89516 5.65625 5.1875C5.65625 6.47984 6.70766 7.53125 8 7.53125C9.29234 7.53125 10.3438 6.47984 10.3438 5.1875C10.3438 3.89516 9.29234 2.84375 8 2.84375ZM8 6.59375C7.22459 6.59375 6.59375 5.96291 6.59375 5.1875C6.59375 4.41209 7.22459 3.78125 8 3.78125C8.77541 3.78125 9.40625 4.41209 9.40625 5.1875C9.40625 5.96291 8.77541 6.59375 8 6.59375Z"
              fill="#6A7A99"
            />
          </g>
          <defs>
            <clipPath id="clip0_108_137">
              <rect width="16" height="16" fill="white" />
            </clipPath>
          </defs>
        </svg>
        <p className="text-dusk text-sm">{location}</p>
      </div>
    </div>
  );
};

export default EventBox;
